...
configure :build do
  ...
  activate :external_pipeline,
    name: :frontend_docs,
    command: "mkdir -p .tmp && cd .tmp && rm -rf frontend && git clone --depth 1 #{config[:frontend_repo_url]} frontend && cd frontend && npm install --ignore-scripts && npm run typedoc -- --out doc/frontend/",
